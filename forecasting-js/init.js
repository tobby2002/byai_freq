...
var url = 'http://127.0.0.1:8007/show1';
...
function completeFnDB(results) {
  ...
  d = results.date;
  o = results.open; h = results.high; l = results.low; c = results.close;
  v = results.volume;

  for(var i = 0;i<rowCount;i++){
      stock_date.push(d[i]);
      // alert(d[i])
      stocks.push([parseFloat(o[i]),
      parseFloat(c[i]),
      parseFloat(l[i]),
      parseFloat(h[i])]);

      close.push(parseFloat(c[i]));
      volume.push(parseInt(v[i]));
  }
