...
// https://stackoverflow.com/questions/24172045/automatically-import-a-local-csv-file-with-javascript-or-jquery
// papaparse
...
function _error(err, file) {
  Materialize.toast("ERROR: " + err + file, 3000)
}

function _complete(results) {
...
function getOption(timestamp_l, oclh_l, volume_l) {
  ...
  // new_oclh = oclh_l.slice()
  // new_volume = volume_l.slice()
  // // when min zone
  // for (var k = 0; k < parseInt(new_date.length/5); k += 1) {
  //   somedate = new Date(new_date[new_date.length - 1])
  //   // somedate.setDate(somedate.getDate() + 1) // when 1day resolution
  //   dd = somedate.getDate()
  ...
  //
  //   // new_date.push(y.toString() + '-' + mm.toString() + '-' + dd.toString()) // when 1day resolution
  //   new_date.push(y.toString() + '-' + mm.toString() + '-' + dd.toString() + ' 00:00:00')
  //   new_oclh.push([])
  //   new_volume.push(0)
  // }
...
$('#trainbutton').click(function () {
  ...
  if (islive) {
    console.log('trainbutton: islive :' + islive)
    // start_live();
    setInterval_stop();
    close_l = rs_dic.close_l
    oclh_l = rs_dic.oclh_l
    timestamp_l = rs_dic.timestamp_l
    volume_l = rs_dic.volume_l
  }
  ...
  setTimeout(function () {
    ...
    cells = [tf.layers.lstmCell({
      units: parseInt($('#sizelayer').val())
    })];
    rnn = tf.layers.rnn({
      cell: cells,
    ...
    function f(x, states) {
      ...
      return {
        'forward': dense_layer.apply(last_sequences),
        'states_1': forward[1],
        'states_2': forward[2]
      }
    }
...
function plot_pairplot(val) {
  ...
  generate_investment(['total investment(%): ', 'total gains: ', 'stock changes: ', 'stock changes (%): ', 'gold changes(%): ', 'crude oil changes(%): '], [total_investment.toFixed(2), total_gain.toFixed(2), stock_changes.toFixed(2),
...
function start_live() {
  ...
  $.ajax({
    ...
    success: function (results) {
      // refer def show_live():
      rs_dic = _get_rs_dic(results);
      index = rs_dic.timestamp_l.length

      if(isfirst || reload) {
        // new_date = rs_dic.timestamp_l.slice()
        // new_oclh = rs_dic.oclh_l.slice()
        // new_volume = rs_dic.volume_l.slice()
        //
        // // when min zone
        // for (var k = 0; k < parseInt(new_date.length/5); k += 1) {
        //   somedate = new Date(new_date[new_date.length - 1])
        ...
        //   new_volume.push(0)
        // }
        option_chart = plot_chart(rs_dic.timestamp_l, rs_dic.oclh_l, rs_dic.volume_l)
        option_chart = plot_chart(new_date, new_oclh, new_volume)
        // opt = option_chart[0]
        cht = option_chart[1]
        isfirst = false
        reload = false
      ...
      // if(reload) {
      //   // option_chart = plot_chart(new_volume, rs_dic.oclh_l, rs_dic.volume_l)
      //   option_chart = plot_chart(new_date, new_oclh, new_volume)
      //   opt = option_chart[0]
      //   cht = option_chart[1]
      //   isfirst = false
      ...
      var this_time = rs_dic.timestamp_last;

      // var last_time = opt.xAxis[0].data.splice(index,1);
      // var this_time = rs_dic.timestamp_last;
      ...
      if (this_time !== last_time) {
        ...
        // opt.xAxis[0].data.push(rs_dic.timestamp_last);
        opt.xAxis[0].data.splice(index, 0, rs_dic.timestamp_last);
        opt.xAxis[1].data.shift();
        // opt.xAxis[1].data.push(rs_dic.timestamp_last);
        opt.xAxis[1].data.splice(index, 0, rs_dic.timestamp_last);
        opt.series[0].data.shift();
        // opt.series[0].data.push(rs_dic.volume_last);
        opt.series[0].data.splice(index, 0, rs_dic.volume_last);
        opt.series[1].data.shift();
        // opt.series[1].data.push(rs_dic.oclh_last);
        opt.series[1].data.splice(index, 0, rs_dic.oclh_last);
        opt.series[2].data.shift();
        opt.series[2].data.push(calculateMA_add(5, rs_dic.oclh_l));
        opt.series[3].data.shift();
        opt.series[3].data.push(calculateMA_add(10, rs_dic.oclh_l));
        opt.series[4].data.shift();
        opt.series[4].data.push(calculateMA_add(20, rs_dic.oclh_l));
        opt.series[5].data.shift();
        opt.series[5].data.push(calculateMA_add(30, rs_dic.oclh_l));
        ...
        // https://www.echartsjs.com/en/tutorial.html#Add%20interaction%20to%20the%20chart%20component
      }
    },
    error: function (data) {
...
function setInterval_start() {
  ...
  st = setInterval(function() { start_live(); }, 3000)
}
