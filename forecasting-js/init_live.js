...
function init_live(topk) {
  console.log('init_live topk:' + topk)
  const url = 'http://127.0.0.1:8007/live/init';
  $.ajax({
    type: 'POST',
    dataType: 'json',
    data: { topk: topk},
    ...
    success: function (results) {
      if (results.data[0]) {
        ...
        if(rs_dic) {
          ...
          opt = option_chart[0]
          cht = option_chart[1]
          lasttime = rs_dic.timestamp_last
          console.log('success init live')
        }
      } else {
        console.log('fail init live')
      }
...
function feed_live() {
  const url = 'http://127.0.0.1:8007/live/feed';
  ...
  $.ajax({
    ...
    url: url,
    success: function (results) {
      console.log('feed live  ----------')
      // option_chart = plot_chart(rs_dic.timestamp_l, rs_dic.oclh_l, rs_dic.volume_l)
      opt = option_chart[0]
      cht = option_chart[1]
      console.log('i:' + i++)
      rs_dic = _get_rs_dic(results)
      console.log('rs_dic:' + rs_dic)

      if (rs_dic) {
          // alert('rs_dic in rs_dic:'+rs_dic)
          ...
          console.log('rs_dic in else:' + rs_dic)
          console.log('adding else')
          console.log('opt.series[1].data.length:' + opt.series[1].data.length)
          console.log('opt.series[1].data:' + opt.series[1].data.slice().join())
          opt.series[1].data.splice(opt.series[1].data.length-1, 0, rs_dic.oclh_last);
          // opt.series[1].data.splice(opt.series[1].data.length-1, 0, null);
          cht.setOption({
            xAxis: [{
                data: opt.xAxis[0].data
              },
...
function _get_rs_dic(results) {
  ...
  console.log('results.data[0] in _get_rs_dic:' + toclhv_l)
  var results_dic = null
  if (toclhv_l) {
      ...
      console.log('results.data[0] in _get_rs_dic:' + results_dic.toString())

  } else {
      console.log('results.data[0] in else _get_rs_dic:' + toclhv_l)
  }
  return results_dic
}
...
function setInterval_start() {
  ...
  st = setInterval(function() { feed_live(); }, 5000) // 5000 5초에 한번씩 실행해서 stick 을 가져옮
}
