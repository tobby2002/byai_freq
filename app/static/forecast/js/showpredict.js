...
// const url = 'http://127.0.0.1:8007/show1';
// const url = 'http://127.0.0.1:8007/show';
//
// $('#calldbbutton').click(function(){
//   $.ajax({
//      type: "POST",
//      dataType: "json",
//      async: false,
...
function completeFn(results) {
  ...
  plot_chart(stocks, stock_date, volume);
}


function completeFnDB(results) {
  var timestamp = results.date;
  var oclh = results.oclh
...
function plot_chart(stocks, stock_date, volume){
  var option = getOption(stocks, stock_date, volume);
  var chart = echarts.init(document.getElementById('div_output'));
  chart.setOption(option, true);
  return [ option, chart ]
}

function getOption(stocks, stock_date, volume) {
...
function add_new_data() {
    ...
    $.ajax({
        ...
        url: url,
        success: function(results){
            console.log('results.data[0].date[0]:' + results.data[0].date[0])
            var rawdata = results.data[0]
            var timestamp = rawdata.date
            var stock_date = timestamp
            var close = rawdata.close
            var stocks = rawdata.oclh
            var volume = rawdata.volume

            console.log('isfirst:' + isfirst)

            if (isfirst == false) {
                option_chart = plot_chart(stocks, stock_date, volume)
                isfirst = true
                console.log('isfirst:' + isfirst)
            }
            // t_ohlc = splitData(results.data);
            ...
            var this_time = timestamp.slice(-1)[0];
            console.log('this_time:' + this_time)

            if (this_time !== last_time) {
            // if (true) {
                opt.xAxis[0].data.shift();
                opt.xAxis[0].data.push(this_time);
                opt.xAxis[1].data.shift();
                opt.xAxis[1].data.push(this_time);
                opt.series[0].data.shift();
                opt.series[0].data.push(volume[0]);
                opt.series[1].data.shift();
                opt.series[1].data.push(stocks[0]);
                opt.series[2].data.shift();
                opt.series[2].data.push(calculateMA_add(5, stocks));
                opt.series[3].data.shift();
                opt.series[3].data.push(calculateMA_add(10, stocks));
                opt.series[4].data.shift();
                opt.series[4].data.push(calculateMA_add(20, stocks));
                opt.series[5].data.shift();
                opt.series[5].data.push(calculateMA_add(30, stocks));  // console.log('stocks[0]:' + stocks[0])  // console.log('opt.series[1].data[0]:' + opt.series[1].data[0])  // console.log('opt.xAxis[0].data:' + opt.xAxis[0].data)
