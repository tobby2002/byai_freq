...
def send_set_password_staff_email(staff_pk):
    _send_set_password_email(staff_pk, "tradingroom/staff/set_password")


@app.task
def send_set_password_customer_email(pk):
    _send_set_password_email(pk, "tradingroom/customer/set_password")
...
def send_promote_customer_to_staff_email(staff_pk):
    ...
    ctx = get_email_base_context()
    ctx["dashboard_url"] = build_absolute_uri(reverse("tradingroom:index"))
    send_templated_mail(
        template_name="tradingroom/staff/promote_customer",
        from_email=settings.DEFAULT_FROM_EMAIL,
