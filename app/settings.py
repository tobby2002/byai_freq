...
DATABASES = {
    "default": dj_database_url.config(
        default="postgres://neo1:@localhost:5432/neo1", conn_max_age=600
    )
}
...
#         'NAME': os.path.join(BASE_DIR, 'saleor_aibitbot_db.sqlite3'),
#     }
# }

# https://chohyeonkeun.github.io/2019/06/20/190620-django-translation/
# https://wikidocs.net/9824
...
STATICFILES_DIRS = [
    ("assets", os.path.join(PROJECT_ROOT, "app", "static", "assets")),
    ("favicons", os.path.join(PROJECT_ROOT, "app", "static", "favicons")),
    ("images", os.path.join(PROJECT_ROOT, "app", "static", "images")),
    (
        "dashboard/images",
        os.path.join(PROJECT_ROOT, "app", "static", "dashboard", "images"),
    ),
    (
        "tradingroom/images",
        os.path.join(PROJECT_ROOT, "app", "static", "tradingroom", "images"),
    ),
    (
        "manager/images",
        os.path.join(PROJECT_ROOT, "app", "static", "manager", "images"),
    ),
]
STATICFILES_FINDERS = [
...
context_processors = [
    ...
    "app.core.context_processors.default_currency",
    "app.checkout.context_processors.checkout_counter",
    "app.core.context_processors.search_enabled",
    "app.site.context_processors.site",
...
MIDDLEWARE = [
    ...
    "app.core.middleware.discounts",
    "app.core.middleware.google_analytics",
    "app.core.middleware.country",
    "app.core.middleware.currency",
    "app.core.middleware.site",
    "app.core.middleware.extensions",
    "social_django.middleware.SocialAuthExceptionMiddleware",
    "impersonate.middleware.ImpersonateMiddleware",
    "app.graphql.middleware.jwt_middleware",
]
...
INSTALLED_APPS = [
    ...
    # Added apps related customed app
    "app.manager",
    "app.coin",
    "app.knowledge",
    "dynamic_models",
...
DEFAULT_COUNTRY = os.environ.get("DEFAULT_COUNTRY", "KR")
...
bootstrap4 = {
    ...
    "set_required": False,
    "success_css_class": "",
    "form_renderers": {"default": "app.core.utils.form_renderer.FormRenderer"},
}
...
if AWS_MEDIA_BUCKET_NAME:
    ...
    DEFAULT_FILE_STORAGE = "app.core.storages.GCSMediaStorage"
    THUMBNAIL_DEFAULT_STORAGE = DEFAULT_FILE_STORAGE
...
SEARCH_BACKEND = "app.search.backends.postgresql"

if ES_URL:
    SEARCH_BACKEND = "app.search.backends.elasticsearch"
    INSTALLED_APPS.append("django_elasticsearch_dsl")
    ELASTICSEARCH_DSL = {"default": {"hosts": ES_URL}}

AUTHENTICATION_BACKENDS = [
    "app.account.backends.facebook.CustomFacebookOAuth2",
    "app.account.backends.google.CustomGoogleOAuth2",
...
IMPERSONATE = {
    ...
    "CUSTOM_USER_QUERYSET": "app.account.impersonate.get_impersonatable_users",  # noqa
    "USE_HTTP_REFERER": True,
    "CUSTOM_ALLOW": "app.account.impersonate.can_impersonate",
}
...
SERIALIZATION_MODULES = {"json": "app.core.utils.json_serializer"}
...
PAYMENT_GATEWAYS = {
    DUMMY: {
        "module": "app.payment.gateways.dummy",
        "config": {
            "auto_capture": True,
            "store_card": False,
            "connection_params": {},
    ...
    BRAINTREE: {
        "module": "app.payment.gateways.braintree",
        "config": {
    ...
    RAZORPAY: {
        "module": "app.payment.gateways.razorpay",
        "config": {
    ...
    STRIPE: {
        "module": "app.payment.gateways.stripe",
        "config": {
...
EXTENSIONS_MANAGER = "app.extensions.manager.ExtensionsManager"
